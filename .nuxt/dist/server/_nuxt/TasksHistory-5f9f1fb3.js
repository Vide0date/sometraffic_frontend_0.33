import { u as useRuntimeConfig, a as useRoute, f as __nuxt_component_0 } from "../server.mjs";
import { useSSRContext, inject, ref, reactive, toRefs, unref, isRef, withCtx, createVNode, createTextVNode, toDisplayString, openBlock, createBlock, withDirectives, vShow } from "vue";
...
const _imports_0 = "" + __buildAssetsURL("right-arrow.b7db5663.png");
const __default__$3 = {
  name: "TasksAll",
  ...
  setup(__props) {
    ...
    const setClickDatas = async () => {
      ...
      if (!localStorage.getItem("activeProject")) {
        ...
        const waitForActiveProject = setInterval(async () => {
          if (localStorage.getItem("activeProject")) {
            ...
            const { data } = await useFetch(
              `${config.API_BASE_URL}tasks/all?${query}&projectId=${localStorage.getItem("activeProject")}`,
              "$q9Se0lvHCP"
            );
        ...
        const { data } = await useFetch(
          `${config.API_BASE_URL}tasks/all?projectId=${localStorage.getItem("activeProject")}`,
          "$R3X9tSnCbw"
        );
        clickdatas.value = data.value.data;
        searchdatas.value = data.value.data;
...
//# sourceMappingURL=TasksHistory-5f9f1fb3.js.map
